...
static void unmanage(pid_t pid) {
    /* remove from array */
    struct procinfo *node = procs_array[pid];

    if (!node)
        return;

    pid_t app_pid = node->app_pid;
    ...
    /* remove from list */
    if (node->prev) {
        struct procinfo *prev = node->prev;
        prev->next = node->next;
    }
    if (node->next) {
        struct procinfo *next = node->next;
        next->prev = node->prev;
    }

    if (node == procs_list)
        procs_list = node->next;

    /* cleanup */
    munmap(node->pdata, sizeof *node->pdata);
    shm_unlink(node->memname);
    free(node->memname);
    free(node);
