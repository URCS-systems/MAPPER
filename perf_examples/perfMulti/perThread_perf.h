...
#define TIME_IN_MILLI_SEC 250  //total measurement time per iteration is 4 times TIME_IN_MILLI_SEC(example 250 millisecond *4 = 1 second)
...
struct perf_stat
{
   ...
   //read format pointers for each buffer (group of events), each group contains two events  
...
void setPerfAttr(struct perf_event_attr pea, uint64_t EVENT,int group_fd,int* fd, uint64_t* id, int cpu, pid_t tid)
{
...
void stop_read_counters( struct read_format* rf, int fd, char* buf,int size, uint64_t* val1, uint64_t* val2, uint64_t id1, uint64_t id2)
{
