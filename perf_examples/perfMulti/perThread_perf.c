...
void count_event_perfMultiplex(pid_t tid[],int index_tid)
{
 ...
 {
          //initialize read format descriptor for each thread and all events for that thread
           int j;
           for(j=0;j<(EVENTS/2);j++)
            ...
            for(j=0;j<(EVENTS/2);j++)
            {    setPerfAttr(threads[i].pea,EVENT[j*2],-1,&(threads[i].fd[(j*2)]),&(threads[i].id[(j*2)]),-1,tid[i]);  //measure tid statistics on any cpu
                setPerfAttr(threads[i].pea,EVENT[j*2+1],threads[i].fd[(j*2)],&(threads[i].fd[(j*2)+1]),&(threads[i].id[(j*2)+1]),-1,tid[i]);
            }
           ...
           //duration of count  
      	  ...
      	  //stop counters and read counter values
             ...
             {   
	          ...
	          stop_read_counters((threads[i].rf[j]),threads[i].fd[j*2],threads[i].buf[j],size,&(threads[i].val[(2*j)]),&(threads[i].val[(2*j)+1]),threads[i].id[(j*2)],threads[i].id[(j*2)+1]);
              }
   }//for j close
...
void copyValues(pid_t tid[], int index_tid)
{
	...
	/*	//FIND PerfData Instance of this particular TID and then populate options_t struct of that 
		THREADS.tid[i]; //TID of the THREAD BEING MONITORED
		THREADS.event[i][0];   //UNHALTED_CORE_CYCLES
