        ...
        for (int i = 0; i < num_counter_orders; ++i) {
            if (an->bottleneck[counter_order[i]] > 0) {
                printf("Added: App %d has bottleneck %s with metric %" PRIu64 "\n",
                        an->pid, metric_names[counter_order[i]], 
                        an->metric[counter_order[i]]);
            }
        }
    }
	return;
...
void PerfData::printCounters(int index)
{
  ...
  //      
	...
	if (options->counters[i].delta > (uint64_t)thresh_pt[i]) {
		...
		bottleneck[i] = 1;
		if (apps_array[apppid])
			apps_array[apppid]->bottleneck[i] += 1;
...
PerfData::~PerfData()
{
	...
	unmanage(pid, apppid);
	std::cout << " Done with unmanage for " << pid << " of application PID " 
			<< apppid << '\n';
...
int main(int argc, char *argv[])
{
	...
	if (init_thresholds == 0) {
	    ...
	    if (cg_create_cgroup(cgroot, cntrlr, "sam") < 0
        	&& errno != EEXIST) {
...
RESUME: 
    ...
    if (num_apps > 0) {
         ...
         * [ apps sorted by bottleneck 1 | apps sorted by bottleneck 2 | ... | remaining ]
         */
        for (int i = 0; i < N_METRICS; ++i) {
            range_ends[i+1] = range_ends[i];
            for (int j = 0; j < num_apps; ++j) {
                if (apps_unsorted[j]
                 && (i >= num_counter_orders 
            ...
            if (i < num_counter_orders) {
                ...
                qsort_r(&apps_sorted[range_ends[i]], range_ends[i+1] - range_ends[i], 
                        sizeof *apps_sorted, 
                        &compare_apps_by_metric, (void*) &met);
            ...
            for (int j = range_ends[i]; j < range_ends[i+1]; ++j) {
                ...
                if (i < num_counter_orders) {
                    ...
                    /*
                     * compute the CPU budget for this application, given its bottleneck [met]
                     */
					if (met == METRIC_INTER || met == METRIC_INTRA) 
						budget_collocate(apps_sorted[j]->cpuset, new_cpuset, remaining_cpus, rem_cpus_sz, per_app_cpu_budget);
					if (met == METRIC_MEM)
						budget_spread(apps_sorted[j]->cpuset, new_cpuset, remaining_cpus, rem_cpus_sz, per_app_cpu_budget);
					if (met == METRIC_AVGIPC)
						budget_no_hyperthread(apps_sorted[j]->cpuset, new_cpuset, remaining_cpus, rem_cpus_sz, per_app_cpu_budget);
                    /* (*budgeter_functions[counter_order[i]])(apps_sorted[j]->cpuset, 
                            new_cpuset, remaining_cpus, rem_cpus_sz, per_app_cpu_budget);*/
                } else {
                    printf("\t[APP %5d] (cpuset = %s)\n", apps_sorted[j]->pid,
                            intlist_to_string(intlist, intlist_l, buf, sizeof buf, ","));

                    budget_default(apps_sorted[j]->cpuset,
                            new_cpuset, remaining_cpus, rem_cpus_sz, per_app_cpu_budget);
                }
                ...
                /* set the cpuset */
                if (mybudget_l > 0) {
                    if (cg_write_intlist(cgroot, cntrlr, cg_name, "cpuset.cpus",
                            mybudget, mybudget_l) != 0) {
