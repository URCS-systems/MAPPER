...
/*
static void manage(pid_t pid) {
    struct perf_data *pdata = NULL;
    int memfd;
    pid_t app_pid, npid;

    if (sscanf(memname, SHMNAME_FMT, &app_pid, &npid) != 2
     || npid != pid) {
        fprintf(stderr, "Invalid format for %d.pid: %s\n", pid, memname);
        free(memname);
        return;
    }

    if ((memfd = shm_open(memname, O_RDWR, 0)) < 0) {
        fprintf(stderr, "Failed to open %s: %s\n", memname, strerror(errno));
        free(memname);
        return;
    }

    pdata = mmap(0, sizeof *pdata, PROT_READ | PROT_WRITE, 
            MAP_SHARED, memfd, 0);
    ...
    struct procinfo *node;

    node = calloc(1, sizeof *node);
    node->app_pid = app_pid;
    node->pid = pid;
    node->memname = memname;
    node->pdata = pdata;
    node->next = procs_list;
    if (procs_list)
        procs_list->prev = node;
    procs_list = node;

    procs_array[pid] = node;

    if (!apps_array[app_pid]) {
        struct appinfo *anode = calloc(1, sizeof *anode);
